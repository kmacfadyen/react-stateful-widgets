...
export default function Counter() {
  /* STEP 1 */
  const [count, setCount] = useState(0); /* Initially equal to 0 (Can be a string too) */

  const increment = () => {
    /* STEP 4  Don't set state directly! It will not work Ex: count++ */
    setCount(count + 1); /* This will work instead! */
  };
  const decrement = () => {
    /* STEP 5 */
    setCount(count - 1);
  ...
  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: count % 2 === 0 ? 'royalblue' : 'crimson' /* STEP 2 (If count is even, return royal blue, if not, return crimson) */
  };
  ...
  return (
    <div className='widget-counter container'>
      ...
      <div id='count' style={style}>
        Number {count} is {count % 2 === 0 ? 'even' : 'odd' } {/* STEP 3 (This is JSX, looks like HTML, but is JS) */}
      </div>
      <div>
